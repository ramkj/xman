...
class SQLiteDataAccessLayer( DataAccessLayer ):
    ...
    def getMonthwisePersonSummary( self ) -> list:
        contents = [] 
        with SQLiteDriver( self.data_file ) as cursor:
            _SQL = """ SELECT SUBSTR( EXPENSE_DATE, 1, 7),  PERSON.PERSON_SHORT_NAME,  COUNT( EXPENSE.ID), SUM( EXPENSE_AMOUNT ) from EXPENSE, PERSON WHERE EXPENSE.PERSON_ID = PERSON.ID GROUP BY SUBSTR( EXPENSE_DATE, 1, 7), PERSON.PERSON_SHORT_NAME ORDER BY EXPENSE_DATE DESC,  PERSON.PERSON_SHORT_NAME  """ 
            cursor.execute( _SQL )
            contents = cursor.fetchall() 
        if contents:
            return contents 
